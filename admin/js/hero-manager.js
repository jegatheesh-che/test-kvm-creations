// ================================================
// KVM Creations — Admin Hero / Slider Manager
// ================================================

import { 
  collection, 
  doc, 
  addDoc, 
  updateDoc, 
  deleteDoc, 
  getDocs, 
  query, 
  orderBy, 
  serverTimestamp 
} from "https://www.gstatic.com/firebasejs/12.16.0/firebase-firestore.js";

import { auth, db } from "../../js/firebase-config.js";
import { uploadImageToCloudinary } from "../../js/cloudinary-service.js";

// Firestore Collection Reference
const HERO_COLLECTION = "hero_slides";

/**
 * Transforms Cloudinary secure_url to dynamic admin 16:9 thumbnail delivery URL (f_auto,q_auto,w_400,h_250,c_fill)
 * @param {string} secureUrl 
 * @returns {string} Transformed thumbnail URL
 */
export function generateHeroThumbnailUrl(secureUrl) {
  if (!secureUrl) return '';
  return secureUrl.replace('/upload/', '/upload/f_auto,q_auto,w_400,h_250,c_fill/');
}

/**
 * Fetches all hero slide documents from Firestore ordered by order ASC
 * @returns {Promise<Array>} List of hero slide document objects with IDs
 */
export async function fetchHeroSlideDocuments() {
  try {
    const q = query(
      collection(db, HERO_COLLECTION),
      orderBy("order", "asc")
    );
    const querySnapshot = await getDocs(q);
    const documents = [];
    querySnapshot.forEach((docSnap) => {
      documents.push({ id: docSnap.id, ...docSnap.data() });
    });
    return documents;
  } catch (error) {
    console.error("Error fetching hero slides from Firestore:", error);
    throw error;
  }
}

/**
 * Uploads slide image file to Cloudinary and saves document in Firestore hero_slides collection
 * @param {Object} slideData - { file, title, order, isActive }
 * @returns {Promise<Object>} Created document object
 */
export async function addHeroSlide(slideData) {
  const { file, title, order, isActive } = slideData;

  if (!file) throw new Error("Please select a widescreen hero image file to upload.");
  if (!title || title.trim() === "") throw new Error("Please provide a slide title.");

  // 1. Upload image to Cloudinary using existing service
  const uploadResult = await uploadImageToCloudinary(file);
  if (!uploadResult.success) {
    throw new Error(`Cloudinary Upload Failed: ${uploadResult.error}`);
  }

  // 2. Prepare Firestore document
  const docData = {
    title: title.trim(),
    imageUrl: uploadResult.secure_url,
    publicId: uploadResult.public_id,
    order: Number(order) || 1,
    isActive: Boolean(isActive),
    createdAt: serverTimestamp()
  };

  // 3. Save to Firestore `hero_slides` collection
  const docRef = await addDoc(collection(db, HERO_COLLECTION), docData);
  return { id: docRef.id, ...docData };
}

/**
 * Quick toggles active state of a hero slide document
 * @param {string} docId 
 * @param {boolean} currentActiveState 
 */
export async function toggleHeroSlideActive(docId, currentActiveState) {
  if (!docId) throw new Error("Missing document ID.");
  const docRef = doc(db, HERO_COLLECTION, docId);
  await updateDoc(docRef, { isActive: !currentActiveState });
}

/**
 * Updates metadata (and optionally replaces image) of an existing hero slide document
 * @param {string} docId 
 * @param {Object} updatedFields - { title, order, isActive, newFile, oldPublicId }
 */
export async function updateHeroSlide(docId, updatedFields) {
  if (!docId) throw new Error("Missing document ID for update.");

  const docRef = doc(db, HERO_COLLECTION, docId);
  const { title, order, isActive, newFile, oldPublicId } = updatedFields;

  const fieldsToUpdate = {
    title: title.trim(),
    order: Number(order) || 1,
    isActive: Boolean(isActive)
  };

  // Case A: Image Replacement
  if (newFile) {
    // 1. Upload new image to Cloudinary
    const uploadResult = await uploadImageToCloudinary(newFile);
    if (!uploadResult.success) {
      throw new Error(`Cloudinary Upload Failed: ${uploadResult.error}`);
    }

    fieldsToUpdate.imageUrl = uploadResult.secure_url;
    fieldsToUpdate.publicId = uploadResult.public_id;

    // 2. Update Firestore with new image credentials
    await updateDoc(docRef, fieldsToUpdate);

    // 3. Only after Firestore update succeeds, delete old image from Cloudinary
    if (oldPublicId) {
      try {
        const currentUser = auth.currentUser;
        if (currentUser) {
          const idToken = await currentUser.getIdToken();
          await fetch('/api/delete-image', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${idToken}`
            },
            body: JSON.stringify({ publicId: oldPublicId, idToken })
          });
        }
      } catch (err) {
        console.warn("Old Cloudinary image cleanup warning:", err);
      }
    }
  } else {
    // Case B: Metadata update only
    await updateDoc(docRef, fieldsToUpdate);
  }
}

/**
 * Safely deletes a hero slide:
 * 1. Authenticates admin & requests Cloudinary deletion via /api/delete-image
 * 2. Only deletes Firestore document if Cloudinary deletion succeeds
 * @param {string} docId - Firestore document ID
 * @param {string} publicId - Cloudinary public_id
 */
export async function deleteHeroSlide(docId, publicId) {
  if (!docId || !publicId) {
    throw new Error("Missing document ID or Cloudinary publicId for deletion.");
  }

  // 1. Ensure authenticated user and fetch Firebase Auth ID token
  const currentUser = auth.currentUser;
  if (!currentUser) throw new Error("Unauthorized: Active admin session required.");
  
  const idToken = await currentUser.getIdToken();

  // 2. Call Vercel Serverless Function /api/delete-image
  const response = await fetch('/api/delete-image', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${idToken}`
    },
    body: JSON.stringify({ publicId, idToken })
  });

  const deleteResult = await response.json();

  if (!response.ok || !deleteResult.success) {
    const errorMsg = deleteResult.error || "Failed to delete image from Cloudinary server.";
    throw new Error(`Cloudinary Deletion Error: ${errorMsg}`);
  }

  // 3. Confirm Cloudinary deletion succeeded before deleting Firestore record
  const docRef = doc(db, HERO_COLLECTION, docId);
  await deleteDoc(docRef);
  
  return { success: true };
}
